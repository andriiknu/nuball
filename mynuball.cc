...
// ********************************************************************
//
// $Id: test.cc 86065 2014-11-07 08:51:15Z gcosmo $
//
/// \file test.cc
/// \brief Main program of test
...
int main(int argc,char** argv)
{
  ...
  runManager->SetUserInitialization(new NuballDetectorConstruction());


  // Physics list
  if ( physicsListName.size() == 0 ) physicsListName = "FTFP_BERT_HP_PEN";
  ...
  runManager->SetUserInitialization(new NuballActionInitialization());
  ...
  if ( macro.size() ) {
    // batch mode
    std::cout << "Just Before Session Start Batch" << std::endl;
    G4String command = "/control/execute ";
    UImanager->ApplyCommand(command+macro);
  }
  else {
    // interactive mode
    std::cout << "Just Before Session Start Interactive" << std::endl;
